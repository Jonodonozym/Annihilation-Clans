...
/**
 * Created by Arjen on 25/12/2015.
 */
public class DatabaseManager {
    ...
    public DatabaseManager(String hostname, int port, String database, String username, String password, Clans plugin) {
        driver = "com.mysql.jdbc.Driver";
        connectionString = "jdbc:mysql://" + hostname + ":" + port + "/" + database + "?user=" + username + "&password="
                + password;
        this.plugin = plugin;
    }

    public DatabaseManager(Clans plugin) {
        this.plugin = plugin;
    }
        ...
        try {
            ...
            try {
                ...
                if (retry && msg.contains("_BUSY")) {
                    ...
                    plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, () -> query(query, false), 20);
                }
            }
            if (statement != null)
                statement.close();
